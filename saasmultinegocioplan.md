...
3. ✅ **Panel `/superadmin`** (`app/superadmin/`): lista de negocios, alta (`nuevo/`), cambiar estado prueba/activo/suspendido, botón "Administrar" que setea cookie `sa_store` y entra a `/admin` en el contexto de ese negocio. `/admin` re-scopeado con `getAdminContext()` / `requireAdminStore()`: `store_admin` ve su `profile.store_id`, `super_admin` ve el negocio elegido. Todas las páginas y server actions de `/admin` (`page`, `pedidos`, `productos*`, `inventario`, `clientes`, `configuracion`, `actions/*`) filtran por `store_id`. Typecheck limpio.
   - ✅ **Branding correcto en todo el sitio** (2026-09-04): se sacó `ACTIVE_CONFIG` hardcodeado de las últimas pantallas que lo tenían (auth, `not-found`, `checkout/exito`, `cuenta/pedidos`) — ahora todas leen el negocio real vía `getStore()` o el hook `useStoreName()` (componentes cliente). `api/webhook` ya venía scopeado por `metadata.store_id`.

4. ✅ **Subida de comprobante + cola `pagos_verificacion`** (2026-09-04): `POST /api/comprobantes` sube la foto/PDF al bucket privado `comprobantes` con `service_role` (funciona para invitados, no depende de que haya sesión). `CheckoutForm.tsx` exige el comprobante antes de habilitar "Confirmar pedido" cuando el método no es tarjeta. `api/checkout/manual` **requiere siempre** `comprobanteUrl`, guarda `orders.comprobante_url`, pone el pedido en `pago_en_revision` y crea la fila en `pagos_verificacion`. Nuevo panel **`/admin/pagos`**: cola con miniatura del comprobante (signed URL, 10 min), datos del pedido/reserva referenciado, y botones Aprobar/Rechazar (`admin/actions/pagos.ts`, factorizado en `lib/pagosVerificacion.ts`) que además actualizan el estado de la tabla de origen (`orders`/`pedidos_restaurante`/`reservaciones`) — sirve tanto para pedidos del sitio web como los que crea el bot de Telegram. `checkout/pendiente` ya no pide "enviar por WhatsApp" — confirma que el comprobante quedó recibido.
   - ✅ **`apps/mobile` con su propio flujo de comprobante** (2026-09-05): `expo-image-picker` (cámara o galería, permisos en `app.json`), sube la foto a `/api/comprobantes` y manda `comprobanteUrl` a `/api/checkout/manual` igual que la web — botón deshabilitado sin foto. Con esto **web y app suben comprobante las dos**, así que `api/checkout/manual` se simplificó de nuevo a exigirlo siempre (sin excepción por canal). Pantalla de confirmación de la app ya no pide "enviar por WhatsApp"; queda como contacto opcional. De paso, fix de un error de tipos preexistente (`theme.gray600`). **Falta:** compilar y probar un build real (`eas build`) — el cambio no se probó en un dispositivo, sólo compila.
   - Nota: quedaron 3 errores de TypeScript preexistentes en `apps/mobile` (`AccountScreen.tsx`, `CartScreen.tsx`, `LoginScreen.tsx` — propiedades de estilo repetidas / colores que no existen en `theme.ts`), sin relación con este cambio — no se tocaron.
   - ✅ **Endpoint + workflow documentado para n8n** (2026-09-04): `POST /api/pagos-verificacion/[id]` protegido con `N8N_SHARED_SECRET` (aprueba/rechaza y cascada el estado — misma lógica que `/admin/pagos`, factorizada en `lib/pagosVerificacion.ts`). Workflow paso a paso en `supabase/README.md` → n8n (Database Webhook de Supabase → Telegram con botones → callback → este endpoint). **Falta armar el workflow en la instancia de n8n real** (no se toca desde este repo — necesita que Diego entre a la UI de n8n).
...
6. Módulo restaurante:
   ...
   - ✅ **Reservar mesa + pedir recoger/domicilio como flujos propios** (2026-09-05): `/carrito` ya no arma el WhatsApp directo — lleva a **`/pedir`** (`PedirForm.tsx`), que pide modalidad (recoger/domicilio, con dirección si aplica) + nombre/teléfono, **crea el registro en `pedidos_restaurante`** (`pendiente_pago`, canal `web`, vía `POST /api/pedidos-restaurante`) y recién ahí muestra el botón de WhatsApp con la referencia — así el pedido queda visible en `/admin/pedidos` aunque el pago se siga coordinando por chat. **`/reservar`** (`ReservarForm.tsx`) hace lo mismo para mesas: nombre/teléfono/fecha/hora/personas → crea la fila en `reservaciones` (`POST /api/reservaciones`) → WhatsApp con la referencia. Enlaces agregados en el Navbar, el footer y la landing del restaurante. Sin pago en la web en ningún caso — coherente con la decisión de Diego (no hay tarjeta real en Ecuador para negocios chicos).
